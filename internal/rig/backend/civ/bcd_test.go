...
func TestEncodeFrequency(t *testing.T) {
	tests := []struct {
		...
		{"one hz", 1, []byte{0x01, 0x00, 0x00, 0x00, 0x00}},
		{"ten hz", 10, []byte{0x10, 0x00, 0x00, 0x00, 0x00}},
		{"hundred hz", 100, []byte{0x00, 0x01, 0x00, 0x00, 0x00}},
		{"160m", 1_840_000, []byte{0x00, 0x00, 0x84, 0x01, 0x00}},
		{"cw watering hole", 14_025_000, []byte{0x00, 0x50, 0x02, 0x14, 0x00}},
		{"odd digits", 14_025_678, []byte{0x78, 0x56, 0x02, 0x14, 0x00}},
		{"6m", 50_313_000, []byte{0x00, 0x30, 0x31, 0x50, 0x00}},
		{"above 100 MHz", 145_500_000, []byte{0x00, 0x00, 0x50, 0x45, 0x01}},
		{"max", maxFrequencyHz, []byte{0x99, 0x99, 0x99, 0x99, 0x99}},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, err := encodeFrequency(tc.hz, false)
			if err != nil {
...
func TestEncodeFrequencyOverflow(t *testing.T) {
	if _, err := encodeFrequency(maxFrequencyHz+1, false); err == nil {
...
func TestFrequencyRoundTrip(t *testing.T) {
	...
	for hz := uint64(0); hz <= maxFrequencyHz; hz += 1_234_567 {
		b, err := encodeFrequency(hz, false)
		if err != nil {
...
func TestDecodeFrequencyInvalid(t *testing.T) {
	tests := []struct {
		...
		// Six bytes is the IC-905's 10 GHz field and is accepted; seven is not
		// a width any Icom sends.
		{"too long", []byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
		{"empty", nil},
...
func TestBCD2(t *testing.T) {
	...
	for _, bad := range [][]byte{nil, {0x00}, []byte{0x00, 0x00, 0x00}, []byte{0x0A, 0x00}, []byte{0x00, 0xF0}} {
